...
public class Link implements ButtonHandler {
    @Override
    public PlainIconButtonElementBuilder forEditor(MdTextFieldElement editor) {
        var btn = net.sayaya.ui.elements.ButtonElementBuilder.button().icon().add(icon().css("fa-sharp", "fa-light", "fa-link"));
        btn.onClick(evt->around(editor,"[", "](url)"));
        return btn;
    }
}
