...
class UrlChangeTest: GwtSpec({
    Given(name="iframe src 관리 컴포넌트 테스트", html="contentUrlChange.html", module="net.sayaya.ContentUrlChange",
        ...
        When("새로운 페이지 이동이 요청되면(contentUrl 변경)") {
            ...
            // history: 1
            Then("부모창의 주소가 변경된다") {
                document.currentUrl shouldEndWith "test1"
            }
            Then("히스토리가 저장된다") {
        ...
        And("페이지를 여러번 이동하면") {
            ...
            // history: 1 -> 2 -> 3 -> 4(*)
            Then("부모창의 주소가 변경된다") {
                document.currentUrl shouldEndWith "test4"
            }
            Then("히스토리가 저장된다") {
        ...
        And("Forward 하면") {
            ...
            // history: 1 -> 2(*) -> 3 -> 4
            Then("부모창의 주소가 변경된다") {
                document.currentUrl shouldEndWith "test2"
            }
            Then("히스토리는 변하지 않는다") {
                js.executeScript("return window.history.length") shouldBe history+4
            }
        ...
        And("페이지를 직접 변경한 다음 다시 돌아오면") {
            document.navigate().to(document.currentUrl.replace("test1", "external-url.html"))
            document.navigate().back()
